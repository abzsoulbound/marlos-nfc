...
export async function POST(req: Request) {
  try {
    ...
    if (action === "assignTable") {
      ...
      if (!tagId || !tableNumber) {
        return NextResponse.json(
          { error: "tagId/tableNumber required" },
          { status: 400 }
        );
      }

      const assignment = setTableAssignment(tagId, tableNumber);
    ...
    if (action === "reassignTag") {
      ...
      if (!oldTagId || !newTagId) {
        return NextResponse.json(
          { error: "oldTagId/newTagId required" },
          { status: 400 }
        );
      }
    ...
    if (action === "mergeSessions") {
      ...
      if (!fromTagId || !intoTagId) {
        return NextResponse.json(
          { error: "fromTagId/intoTagId required" },
          { status: 400 }
        );
      }
